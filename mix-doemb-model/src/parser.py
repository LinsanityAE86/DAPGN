...
class Parser(object):
    ...
    def init_models(self):
        ...
        if self._conf.is_shared_lstm:
            ...
            self.add_lstm(2, lstm_input_size, 1)
            self.add_lstm(3, lstm_input_size_domain, 3)
    ...
    def forward(self, words, ext_words, tags, masks, domains, domain_id, word_lens, chars_i, unlabel=False):
        ...
        if unlabel == False:
            ...
            x_domain = x_domain.transpose(0,1)
            x_input_final = torch.cat((input_out, x_domain), dim=2)
            ...
            private_lstm_out = self._lstm_layer[3](x_input_final, lstm_masks, initial=None, is_training=is_training)
    ...
    def train_set_label(self, bc, pc, pb, zx,domain, domain_src, domain_tgt, dataset, unlabel, eval_iter = -1):
        if (domain_tgt < bc):
            domain =0
        elif (bc <= domain_tgt < bc+pc):
            domain = 1
        elif (bc+pc <= domain_tgt < bc+pc+pb):
            domain = 2
        else:
            domain = 3
    ...
    def train(self):
        ...
        bc,pc,pb,zx=self._train_datasets[0].batch_num,self._train_datasets[1].batch_num, self._train_datasets[2].batch_num,self._train_datasets[3].batch_num
        label_batch_num = bc+pc+pb+zx
        ...
        while True:
            if eval_cnt <= 30:
                if (train_iter < label_batch_num):# or udomain_src >= unlabel_batch_num): 
                #if (train_iter % 2 == 0):# or udomain_src >= unlabel_batch_num): 
                    domain_src, domain_tgt, domain, inst_num, loss = self.train_set_label(bc,pc, pb, zx, domain, domain_src, domain_tgt, self._train_datasets, unlabel=False)
            ...
            if eval_cnt >30:#(if =20, evalate las at 20 iters)
                use_unlabel =False
